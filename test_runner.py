...
class TestHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        self.data = self.request.recv(1024).strip().decode(FORMAT)
        ...
        if command == "ping":
            ...
            print(time.time())
            self.request.sendall("pong".encode(FORMAT))
        elif command == "runtest":
            print ("got runtest command: am I busy? %s" % self.server.busy)
            if self.server.busy:
                self.request.sendall("BUSY".encode(FORMAT))
            else:
    ...
    def run_tests(self, commit_hash, repo_folder):
        ...
        helpers.communicate((self.server.dispatcher_server["host"]).encode(FORMAT),
                            int(self.server.dispatcher_server["port"]),
                            ("results:%s:%s:%s" % (commit_hash, len(output), output)).encode(FORMAT))
...
def serve():
    ...
    response = helpers.communicate((server.dispatcher_server["host"]).encode(FORMAT),
                                   int(server.dispatcher_server["port"]),
                                   (f"register:{runner_host}:{runner_port}").encode(FORMAT))
            ...
            if (time.time() - server.last_communication) > 10:
                try:
                    response = helpers.communicate(
                                       (server.dispatcher_server["host"]).encode(FORMAT),
                                       int(server.dispatcher_server["port"]),
